...
#/opt/riscv32im/bin/riscv32-unknown-elf-gcc -Wa,-adhln -g -T$LDS_FILE $OPT_LEVEL $RVCFLAGS -I$INCLUDE_DIR $FILE $RVGCC_LIB $START_FILE > $FILEwoSUFFIX.S
#./makehex.py firmware.bin 8192  > firmware.hex
./makehex.py firmware.bin $((8192*4)) > firmware.hex
...
#/opt/riscv32im/bin/riscv32-unknown-elf-gcc $OPT_LEVEL $RVCFLAGS  $FILE -Wl,-gc-sections,--strip-debug -o firmware.elf
#/opt/riscv32im/bin/riscv32-unknown-elf-ld	$RVLDFLAGS -T$LDS_FILE main_mandel_float.o -o firmware.elf $RVGCC_LIB
#/opt/riscv32im/bin/riscv32-unknown-elf-strip --strip-all firmware.elf
#/opt/riscv32im/bin/riscv32-unknown-elf-objcopy -O binary firmware.elf firmware.bin
